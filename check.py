...
def deleteAllUsers(admin_password):
    ...
    try:
        with conn:
            ...
            if admin:
                print("Admin password is correct, deleting all users.")
...
def visible(event):
    ...
    if current == '*':
        ...
        vis.config(text=f"◡", font="Calibri 14 bold")
    else:
        entry2.configure(show='*')
        vis.config(text=f"👁", font="Calibri 12")
app = Tk()
app["bg"] = "#FFFFFF"
...
b = Label(image=img, background="#FFFFFF")
b.image = img
b.pack(pady=10)
btn_style = ttk.Style()
btn_style.configure("Rounded.TButton",
                    ...
                    focuscolor="#FFFFFF",
                    padding=6)
btn_login = Button(app, text="Enter", bg="#FFFFFF", font=("Calibri", 10), command=inputLogin, relief="flat", borderwidth=1)
btn_login.place(relx=0.25, rely=0.25, x=-5, y=200, width=160, height=35)
btn_register = Button(app, text="Are you not with us yet?", bg="#FFFFFF", font=("Calibri", 10), command=inputRegister, relief="flat", borderwidth=1)
btn_register.pack(side=BOTTOM, pady=30)
Hovertip(btn_register,
         "To register, you need to enter\nlogin in the first field, and for the password - in the second field\n",
         hover_delay=70)
btn_delete = Button(app, text="Delete User", bg="#FFFFFF", font=("Calibri", 10), command=inputDelete, relief="flat", borderwidth=1)
btn_delete.place(relx=0.25, rely=0.25, x=-5, y=150, width=160, height=35)
btn_delete_all = Button(app, text="Delete All Users", bg="#FFFFFF", font=("Calibri", 10), command=adminDeleteAll, relief="flat", borderwidth=1)
btn_delete_all.place(relx=0.25, rely=0.25, x=-5, y=110, width=160, height=35)
label1 = ttk.Label(text="Login", font=("Calibri", 11, "bold"), background="#FFFFFF")
label1.place(x=69, y=67, width=150)
entry1 = ttk.Entry(app)
entry1.place(anchor=CENTER, relx=0.25, rely=0.25, x=70, y=0, width=150)
label2 = ttk.Label(text="Password", font=("Calibri", 11, "bold"), background="#FFFFFF")
...
vis = ttk.Label(app, text=f"👁", background="#FFFFFF", font=("Calibri", 12))
